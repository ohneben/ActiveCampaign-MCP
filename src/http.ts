...
export function startupRefusal(cfg: HttpConfig): string | undefined {
  ...
  return (
    ...
    `write and delete real ActiveCampaign account data, so it must not be exposed without ` +
    `authentication.\n\n` +
    `Pick one:\n` +
...
export function weakTokenWarning(cfg: HttpConfig): string | undefined {
  ...
  return (
    ...
    `This endpoint can read, write and delete ActiveCampaign account data and is the only ` +
